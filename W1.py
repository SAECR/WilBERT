...
model = RobertaModel.from_pretrained("./Model_Wilbert_3", return_dict=False)
tokenizer = RobertaTokenizer.from_pretrained("./Model_Wilbert_3/Tokenizer")

max_len = 100

class ECRReportDataset(Dataset):
...
def sample(seed_list):
    ...
    for i in seed_list:
        class SentimentClassifier(nn.Module):
            def __init__(self, n_classes):
                ...
                self.robbert = RobertaModel.from_pretrained("./Model_Wilbert_3", return_dict=False)
        ...
        wilbert = SentimentClassifier(len(class_names))
        wilbert = wilbert.to(device)
        ...
        for epoch in range(EPOCHS):
            ...
            if val_acc > best_accuracy:
                torch.save(model.state_dict(), 'W1_best_model_state.bin')
        ...
        plt.title('Training history WilBERT 1')
        plt.ylabel('Accuracy')
        plt.xlabel('Epoch')
        ...
        plt.legend()
        plt.ylim([0, 1]);
        plt.savefig(f'./Output/W1_e{EPOCHS}b{BATCH_SIZE}lr{lr}s{i}ts{test_size}acc{acc}.png')
        ...
        plt.title('Training loss WilBERT 1')
        plt.ylabel('Cross Entropy loss')
        plt.xlabel('Epoch')
        epochs = list(range(EPOCHS))
        plt.xticks(epochs)
        plt.ylim([0, 1]);
        plt.savefig(f'./Output/W1_loss_e{EPOCHS}b{BATCH_SIZE}lr{lr}s{i}ts{test_size}acc{acc}.png')
        ...
        plt.savefig(f'./Output/W1_CFM_e{EPOCHS}b{BATCH_SIZE}lr{lr}s{i}ts{test_size}acc{acc}.png')
        cmstats = ConfusionMatrix(actual_vector=y_test.tolist(), predict_vector=y_pred.tolist())
        print(cmstats.stat())

        pred_test_df = pd.DataFrame(y_report_texts)
        pred_test_df['preds'] = y_pred.tolist()
        pred_test_df['actual'] = y_test.tolist()
        pred_test_df.to_excel(f'./Output/W1_e{EPOCHS}b{BATCH_SIZE}lr{lr}s{i}ts{test_size}acc{acc}.xlsx')
    ...
    def show_confusion_matrix(confusion_matrix):
        ...
        plt.ylabel('True sentiment')
        plt.xlabel('Predicted sentiment')
        plt.title('Confusion matrix WilBERT 1')
        plt.subplots_adjust(bottom=0.05);
    cm = confusion_matrix(cm_act, cm_pred)
    df_cm = pd.DataFrame(cm, index=class_names, columns=class_names)
    show_confusion_matrix(df_cm)
    plt.savefig(f'./Output/W1_CFM_e{EPOCHS}b{BATCH_SIZE}lr{lr}s{10}ts{test_size}acc{sum(results)/len(results)}.png',bbox_inches='tight')
