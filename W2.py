...
model = RobertaModel.from_pretrained("./Model_Extend_Wilbert_2", return_dict=False)
tokenizer = RobertaTokenizer.from_pretrained("./Model_Extend_Wilbert_2/Tokenizer")

max_len = 100

class ECRReportsDataset(Dataset):
...
def sample(seed_list):
    ...
    for i in seed_list:
        class SentimentClassifier(nn.Module):
            def __init__(self, n_classes):
                ...
                self.robbert = RobertaModel.from_pretrained("./Model_Extend_Wilbert_2", return_dict=False)
        ...
        for epoch in range(EPOCHS):
            ...
            if val_acc > best_accuracy:
                torch.save(model.state_dict(), 'WE2_best_model_state.bin')
        ...
        plt.title('Training history WilBERT 2')
        plt.ylabel('Accuracy')
        plt.xlabel('Epoch')
        epochs = list(range(EPOCHS))
        plt.xticks(epochs)
        plt.ylim([0, 1]);
        plt.savefig(f'./Output/W2_e{EPOCHS}b{BATCH_SIZE}lr{lr}s{i}ts{test_size}acc{acc}.png')
        ...
        plt.title('Training loss WilBERT 2')
        plt.ylabel('Cross Entropy loss')
        plt.xlabel('Epoch')
        epochs = list(range(EPOCHS))
        plt.xticks(epochs)
        plt.ylim([0, 1]);
        plt.savefig(f'./Output/W2_loss_e{EPOCHS}b{BATCH_SIZE}lr{lr}s{i}ts{test_size}acc{acc}.png')
        ...
        def show_confusion_matrix(confusion_matrix):
            ...
            plt.title('Confusion matrix WilBERT 2')
            plt.subplots_adjust(bottom=0.05);
        cm = confusion_matrix(y_test, y_pred)
        df_cm = pd.DataFrame(cm, index=class_names, columns=class_names)
        show_confusion_matrix(df_cm)
        plt.savefig(f'./Output/W2_CFM_e{EPOCHS}b{BATCH_SIZE}lr{lr}s{i}ts{test_size}acc{acc}.png', bbox_inches = 'tight')

        pred_test_df = pd.DataFrame(y_report_texts)
        pred_test_df['preds'] = y_pred.tolist()
        pred_test_df['actual'] = y_test.tolist()
        pred_test_df.to_excel(f'./Output/W2_e{EPOCHS}b{BATCH_SIZE}lr{lr}s{i}ts{test_size}acc{acc}.xlsx')
    ...
    df_cm = pd.DataFrame(cm, index=class_names, columns=class_names)
    show_confusion_matrix(df_cm)
    plt.savefig(f'./Output/W2_CFM_e{EPOCHS}b{BATCH_SIZE}lr{lr}s{10}ts{test_size}acc{sum(results)/len(results)}.png',bbox_inches='tight')
